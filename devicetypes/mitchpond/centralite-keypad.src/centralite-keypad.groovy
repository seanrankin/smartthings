	...
	if (description?.startsWith('catchall:')) {
		...
		if (message?.isClusterSpecific == false) {
			...
			//------Read attributes responses------//
				...
				logdebug "Unhandled profile-wide command: "+description;
		}
		//------Cluster specific commands------//
			...
			else logdebug "Unhandled cluster-specific command: "+description
		}
	}
...
private Map parseIasMessage(String description) {
    ...
    switch(msgCode) {
        ...
        case '0x0004':
            ...
            break;
        default:
        	logdebug "Invalid message code in IAS message: ${msgCode}"
    }
    return resultMap
}
...
/*  May 08, 2019 platform changed ST does not send createEvent or sendEvent with data */
//	createEvent(name: "codeEntered", value: keycode as String, data: armMode as String,
//				isStateChange: true, displayed: false)
/*  tried this but ends up on other side as {code=0000, mode=2} a pain to process so not bothering */
...
private sendStatusToDevice() {
	logdebug 'Sending status to device...'
	def armMode = device.currentValue("armMode")
	logtrace 'Arm mode: '+armMode
	def status = ''
...
private sendRawStatus(status, seconds = 00) {
	logdebug "Sending Status ${zigbee.convertToHexString(status)}${zigbee.convertToHexString(seconds)} to device..."
